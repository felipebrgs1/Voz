      ...
      <div class="flex h-16 items-center justify-between">
        ...
        <div class="hidden md:flex items-center space-x-8 flex-1 justify-center">
          <button v-for="item in menuItems" :key="item" @click="navigateToPage(item.toLowerCase())"
            class="px-4 py-2 rounded-lg text-gray-300 hover:bg-gray-500 hover:text-indigo-600 transition-colors">
            {{ item }}
          </button>
        </div>

        <div class="hidden md:flex items-center space-x-4">
          <button class="px-4 py-2 rounded-lg text-gray-300 hover:bg-gray-500 hover:text-indigo-600 transition-colors">
            Entrar
          </button>
          <img src="/user.svg" alt="Entrar" class="h-10 w-auto cursor-pointer">
        </div>
...
const handleScroll = () => {
  if (window.scrollY > 50) {
    navbarClass.value = 'text-gray-300 bg-black/30'
  } else {
    navbarClass.value = 'text-gray-300'
  }
}

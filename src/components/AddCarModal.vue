<template>
  <div class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75 z-50"
    ...
    <div class="bg-white p-8 rounded-lg shadow-lg w-full max-w-3xl">
      <h3 class="text-xl font-bold mb-4">{{ isEditing ? 'Editar Carro' : 'Adicionar Novo Carro' }}</h3>
      <form @submit.prevent="handleSubmit">
        <div><img class="mx-auto mb-4" alt="" :src="carStore.newCar.imageurl"></div>
        <div class="mb-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="mb-4">
            ...
            <input type="number" id="velocidademaxima" v-model="carStore.newCar.velocidademaxima"
              class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              required />
          </div>
          <div class="mb-4">
            <label for="potencia" class="block text-gray-700 font-bold mb-2">Potência</label>
            <input type="number" id="potencia" v-model="carStore.newCar.potencia"
              class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              min="0" max="10" required />
          </div>
          <div class="mb-4">
            <label for="conforto" class="block text-gray-700 font-bold mb-2">Conforto</label>
            <input type="number" id="conforto" v-model="carStore.newCar.conforto"
              class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              min="0" max="10" required />
          </div>
        </div>
